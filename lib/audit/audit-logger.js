...
const ACTION_TYPES = [
  ...
  'heredoc_bypass_observed',   // ENH-390 (v2.1.33) — warning-tier heredoc seen and ALLOWED.
                               // Distinct from _blocked so the audit trail does not
                               // report a permitted command as a prevented one.
  'git_push_intercepted',      // ENH-298 — push-event-guard ask/deny verdict
  /*
   * ENH-303's `post_tool_block_recorded` was removed in v2.1.37 (ENH-432).
   *
   * Nothing ever wrote it — the only references outside this list were tests
   * asserting the list contained it. It existed for a block decision bkit cannot
   * make: `continueOnBlock` is a field on a PROMPT-type hook definition, and all
   * 28 bkit handlers are `"type": "command"`. An action type that can never
   * appear in the ledger teaches a reader to look for evidence that does not
   * exist.
   */
