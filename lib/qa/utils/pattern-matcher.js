...
function extractRequires(content) {
  ...
  /*
   * v2.1.34 — comments are prose, not code.
   *
   * This scanned every line, including comments, so a JSDoc block documenting
   * a module's calling site —
   *
   *     Calling site (scripts/unified-bash-pre.js):
   *       const { detect } = require('../lib/defense/heredoc-detector');
   *
   * — was read as a real require, resolved relative to the WRONG file, and
   ...
   * contains `require('./foo')` and `require("../lib/x.js")` as examples.
   *
   ...
   * reported BLOCKED on every run and could never reach L1. A gate that is
   * permanently red is as uninformative as one that is permanently green, and
   * it hid the sixth finding, which was real.
   *
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (;;) {
      ...
      if (close === -1) { line = line.slice(0, open); inBlockComment = true; break; }
      line = line.slice(0, open) + line.slice(close + 2);
    }

    // Strip a line comment, but only when `//` is not inside a string literal —
    // `require('http://x')` must survive.
    const lineComment = line.search(/(^|[^:'"\w])\/\//);
