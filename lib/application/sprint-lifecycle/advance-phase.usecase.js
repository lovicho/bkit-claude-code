...
async function advancePhase(sprint, toPhase, deps) {
  ...
  const updated = cloneSprint(sprint, {
    phase: toPhase,
    // ENH-417 (v2.1.33): keep `status` consistent with the terminal phase.
    //
    // `archiveSprint` sets both `phase` and `status` to 'archived', but this
    // path — `/sprint phase <id> --to archived` — only ever wrote `phase`, so a
    // sprint could sit at phase 'archived' with status 'active' indefinitely.
    // Measured 2026-08-08: 6 of 7 sprint state files were in exactly that shape.
    // The consequence was not cosmetic — `latestActiveSprint()` selects on
    // `status === 'active'`, so the sprint Stop hook's fallback kept resurrecting
    // long-finished sprints and reporting them as if they had just advanced.
    status: toPhase === 'archived' ? 'archived' : sprint.status,
    phaseHistory: opened,
