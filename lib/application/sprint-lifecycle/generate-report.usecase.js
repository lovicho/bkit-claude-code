 ...
 *
 * Every registered gate appears. A gate with no measurement shows as
 * "not measured" rather than being omitted — see the note in the loop.
 */
function renderQualityGatesSection(sprint) {
  ...
  let passedCount = 0;
  /*
   * v2.1.34 — an unmeasured gate is RENDERED, not skipped.
   *
   * Dropping every gate with a null  meant a sprint that measured
   * nothing produced a Quality Gates table containing only the gates that
   * happened to have numbers, under a heading reading "N gates, N passed".
   * A reader cannot tell that from a sprint where every gate genuinely passed —
   * the report asserted completeness it had not earned, which is the same
   * fail-open as the gate itself reporting 100 when it had measured nothing.
   *
   * An unmeasured gate now shows as "not measured" and counts against the
   * passed tally, because it is not a pass.
   */
  let unmeasuredCount = 0;
  for (const [key, gate] of Object.entries(qg)) {
    if (!gate) continue;
    const gateName = key.split('_')[0];
    const measured = gate.current !== null && gate.current !== undefined;
    if (!measured) unmeasuredCount++;
    const current = measured ? gate.current : 'not measured';
    const passed = measured ? (gate.passed ? '✓' : '✗') : '—';
    if (measured && gate.passed) passedCount++;
    const lastMeasured = gate.lastMeasuredAt || '—';
    const source = gate.source || gate.measuredBy || '—';
    rows.push(`| ${gateName} | ${current} | ${gate.threshold} | ${passed} | ${lastMeasured} | ${source} |`);
  }
  // Only a sprint with no registered gates at all has nothing to render.
  if (rows.length === 0) return null;
  return [
    `## Quality Gates (${rows.length} gates, ${passedCount} passed`
      + `${unmeasuredCount > 0 ? `, ${unmeasuredCount} not measured` : ''})`,
    '',
