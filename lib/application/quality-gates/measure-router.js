...
/**
 * ENH-475 (v2.1.37) — append the fork-mode lead when it has not been mitigated.
 *
 * @param {string} base
 * @returns {string}
 */
...
function parseAgentOutput(agentResult) {
  /*
   * ENH-475 (v2.1.37): these two are the shapes fork mode produces.
   *
   * On Claude Code v2.1.232+ a subagent's result arrives as a notification in a
   * LATER turn, and this router awaits it inside the turn that spawned it, so
   * what lands here is empty. `no_output` carried no error text at all and
   * `no_json` blamed the agent's formatting — both send the reader looking for a
   * bug in an agent that never spoke. The advisory names the likely upstream
   * cause and the remedy, and suppresses itself when the user has already turned
   * fork mode off. See lib/domain/policy/fork-mode-advisory.js.
   */
  if (!agentResult || typeof agentResult.output !== 'string' || agentResult.output.trim() === '') {
    return {
      ok: false,
      reason: 'no_output',
      error: forkModeHint('The agent returned no output, so this gate was not measured.'),
    };
  }
  ...
  if (!block) {
    return {
      ok: false,
      reason: 'no_json',
      error: forkModeHint('No balanced JSON object was found in the agent output, '
        + 'so this gate was not measured.'),
    };
  }
  let parsed;
  ...
  /*
   * ENH-412 (v2.1.33): bound the value as well as type-check it.
   *
   * The finite check above already rejects NaN and Infinity, which is the half
   * that `gap-detector.adapter.js` was missing. The other half was missing
   * here: an out-of-range figure passed through untouched, so an agent
   * reporting 999 would clear every percentage threshold at once — and these
   * values are agent-authored, which is exactly the input that should not be
   * trusted to stay in range. Gates are percentages; clamp to 0..100.
   */
  const value = Math.min(100, Math.max(0, parsed.value));
  return {
    ok: true,
    value,
