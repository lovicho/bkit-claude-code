...
/*
 * Three-state match-rate rules. Required eagerly (not lazily) because the
 * transition table below is built at module load and its `when` predicates
 * close over it — a lazy getter would be evaluated per call for no benefit.
 */
...
const PDCA_PHASE_TRANSITIONS = {
  ...
  /*
   * v2.1.34: three branches, not two.
   *
   * These read `ctx.matchRate >= 90` and `ctx.matchRate < 90`. With matchRate
   * now legitimately null when nothing was measured, the second was TRUE for
   * null — so an unmeasured feature was routed to auto-repair and told "Check
   * below threshold", spending up to five iteration cycles fixing a gap nobody
   * had found. An unmeasured rate is neither branch: the answer is to measure.
   */
  'check': {
    conditions: [
      {
        when: (ctx) => matchRateRules.allowsAdvance(ctx.matchRate, 90),
        next: 'report',
        skill: '/pdca report',
      ...
      {
        when: (ctx) => matchRateRules.isMeasuredFailure(ctx.matchRate, 90),
        next: 'act',
        skill: '/pdca iterate',
      ...
      {
        when: (ctx) => !matchRateRules.isMeasured(ctx.matchRate),
        next: 'check',
        skill: '/pdca analyze',
        message: 'No match rate was measured. Run gap analysis before advancing — '
          + 'an unmeasured gate is not a passing one.',
        taskTemplate: '[Check] {feature}'
      }
    ]
  },
