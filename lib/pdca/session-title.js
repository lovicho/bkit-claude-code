 ...
 */
/**
 * Did bkit write this title? (ENH-419)
 *
 * Only titles bkit produced may be replaced by bkit. The check is deliberately
 * shape-based rather than exact-match: the format is user-configurable via
 * `ui.sessionTitle.format`, and a title carrying the `·<tag>` suffix or the
 * default `[bkit]` marker is one this module emitted.
 *
 * @param {string} title
 * @returns {boolean}
 */
function isBkitTitle(title) {
  if (typeof title !== 'string') return false;
  return /^\[bkit\]/.test(title) || /\s·[0-9a-f]{4}$/.test(title);
}
...
function generateSessionTitle(opts = {}) {
  const { action, feature: optFeature, phase: optPhase, sessionId, currentTitle } = opts;

  /*
   * ENH-419 (v2.1.33), Issue #77 — never overwrite a title the user chose.
   *
   * Claude Code supplies the current title to hooks as `session_title`, and its
   * documentation names this exact use:
   *
   *   "The current session title if one is already set, for example via
   *    `--name` or `/rename`. A hook that emits `sessionTitle` can check
   *    `session_title` first to avoid overwriting a title the user set
   *    explicitly."   — code.claude.com/docs/en/hooks.md:1039
   *
   * bkit never read it. Verified empirically on CC 2.1.226: both SessionStart
   * and UserPromptSubmit carry `session_title`, so the information was there
   * the whole time. Issue #77 ("bkit overwrites the automatic session title on
   * every message") was closed in v2.1.21 by adding a per-session tag, which
   * addressed collisions between parallel windows but not the overwriting —
   * users kept renaming sessions and watching bkit rename them back.
   *
   * A title bkit did not write is the user's, or Claude Code's own summary of
   * the conversation. Either way it is better than a PDCA phase label, and
   * bkit stays out of the way.
   */
  if (typeof currentTitle === 'string' && currentTitle.length > 0 && !isBkitTitle(currentTitle)) {
    return undefined;
  }
  ...
  // v2.1.34: `currentPhase` is a v1 schema key; the phase lives on the feature
  // entry. The line above already reads `primaryFeature` correctly.
  const phase = optPhase
    || (feature && pdcaStatus && pdcaStatus.features
      && pdcaStatus.features[feature] && pdcaStatus.features[feature].phase)
    || undefined;
