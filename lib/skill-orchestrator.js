...
function getSkillConfig(skillName) {
  ...
  try {
    ...
    const config = {
      ...
      // ENH-434 (v2.1.37): `skills_preload` was read here and consumed nowhere,
      // while four agents declared it in their frontmatter believing it preloaded
      // skills. It never did — Claude Code's field is `skills`, `skills_preload`
      // appears nowhere in its documentation, and this reader only looked at SKILL
      // frontmatter anyway, which no skill ever used. A reader for a key nothing
      // writes and nothing consumes is what made the field look alive; removing it
      // with the four declarations keeps them from being reintroduced together.
