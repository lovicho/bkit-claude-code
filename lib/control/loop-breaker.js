...
function recordAction(actionType, target) {
  ...
  switch (actionType) {
    ...
    default: {
      // ENH-378: an unrecognised actionType used to fall out of this switch
      // silently, so a caller could believe it was feeding a rule when nothing
      // consumed the value. `scripts/unified-bash-post.js` did exactly that with
      // 'bash_command' from v2.0.0 until v2.1.32. Surfacing it makes the next
      // mis-wiring visible instead of invisible.
      try {
        require('../core/debug').debugLog('LoopBreaker', 'recordAction ignored: no rule consumes this actionType', {
          actionType,
        });
      } catch (_) { /* debug logging must never break a hook */ }
      break;
    }
  }
}
 ...
 */
/**
 * Apply the configured loop limits over the defaults, once.
 *
 * ENH-455 (v2.1.36) — the three `guardrails.loopBreaker.*` limits shipped in
 * bkit.config.json carrying exactly the numbers the table above hardcodes, so
 * the file read as the place to tune them while changing nothing. Lowering
 * `maxSameFileEdits` to 3 had no effect and nothing said so.
 *
 * Deliberately lazy rather than resolved at module load. `lib/core/config`
 * memoises the parsed file under one global key, so a load-time read captures
 * whatever working directory the FIRST require happened to run in — and tests
 * that `process.chdir()` into a fixture directory would poison that cache for
 * everything loaded afterwards. Measured: doing this at load time turned
 * control-pipeline CP-010 red in the full suite while it stayed green
 * standalone, which is exactly the signature of order-dependent state.
 *
 * A non-positive or non-integer configured value is ignored rather than
 * applied: a malformed config must not be able to set a loop limit to 0 and
 * turn the breaker into a permanent trip. `warnAt` is rescaled with the same
 * ratio setThreshold() uses, so a raised ceiling does not leave the warning at
 * a threshold above it.
 */
let _limitsApplied = false;
function _applyConfiguredLimits() {
  if (_limitsApplied) return;
  _limitsApplied = true;

  const MAP = [
    ['LB-001', 'guardrails.loopBreaker.maxPdcaIterations'],
    ['LB-002', 'guardrails.loopBreaker.maxSameFileEdits'],
    ['LB-003', 'guardrails.loopBreaker.maxAgentRecursion'],
  ];

  let getConfig;
  try { ({ getConfig } = require('../core/config')); } catch (_) { return; }

  for (const [ruleId, key] of MAP) {
    const rule = LOOP_RULES[ruleId];
    if (!rule) continue;
    let v;
    try { v = getConfig(key, rule.maxCount); } catch (_) { continue; }
    if (typeof v !== 'number' || !Number.isInteger(v) || v <= 0) continue;
    if (v === rule.maxCount) continue;
    const ratio = (typeof rule.warnAt === 'number' && rule.maxCount > 0)
      ? rule.warnAt / rule.maxCount
      : null;
    rule.maxCount = v;
    if (ratio !== null) rule.warnAt = Math.max(1, Math.round(v * ratio));
  }
}

function checkLoop() {
  _applyConfiguredLimits();
...
function reset(scope, target) {
  switch (scope) {
    ...
    case 'feature':
      if (target) {
        // ENH-378: clear the persisted bucket too. The H3 migration moved
        // LB-001's counter into a cross-process file and updated `reset('all')`
        // and `resetCounter()` to delete from it, but this branch kept clearing
        // only the in-process mirror. Since `getCounters()` reports the
        // persisted bucket, resetting one feature appeared to do nothing — the
        // long-standing LB-013 unit-test failure.
        try {
          lockedUpdate(_counterFilePath(), (raw) => {
            const next = raw && typeof raw === 'object' ? raw : {};
            if (next.pdcaIterations && typeof next.pdcaIterations === 'object') {
              delete next.pdcaIterations[target];
            }
            return next;
          });
        } catch (_) { /* no persisted state to clear — graceful */ }
