...
function emergencyStop(reason) {
  ...
  /*
   * ENH-453 (v2.1.36) — `automation.emergencyStopEnabled` shipped in
   * bkit.config.json and nothing read it, while this function read
   * `automation.emergencyFallbackLevel`, which the config never shipped. Both
   * halves of one feature were disconnected from the file that claims to
   * configure them. Both keys are now real: the switch gates the drop, and the
   * fallback level is the shipped value.
   *
   * Disabled means "do not change the level", not "ignore the emergency" — the
   * caller's own handling and the audit trail are untouched. A guard that can be
   * silently neutralised is the defect this release exists to remove.
   */
  if (getConfig('automation.emergencyStopEnabled', true) === false) {
    debugLog('automation-controller', 'emergency stop disabled by config; level unchanged', { reason });
    return { previousLevel, fallbackLevel: previousLevel, applied: false };
  }
  ...
  return { previousLevel, fallbackLevel, applied: true };
}
