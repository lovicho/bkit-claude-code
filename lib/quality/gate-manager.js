...
function checkGate(phase, context) {
  ...
  /*
   * v2.1.34: a metric that was never measured blocks, and says so.
   *
   * `_evaluateCondition` returns false for an absent metric, so an unmeasured
   * required metric satisfied neither the pass condition nor the hard-fail
   * condition. Measured: an unmeasured matchRate produced `verdict: 'retry'`
   * with `blockers: []` — the gate declined to advance but gave no reason, and
   * a user reading that sees a score of 50 and an empty explanation.
   *
   * Since matchRate can now legitimately be null (nothing compared), this is
   * the difference between "your code scored badly" and "bkit never looked".
   * Both stop the workflow; only one tells you to go and measure.
   */
  const unmeasured = gate.pass
    .map((cond) => cond.metric)
    .filter((metric) => metrics[metric] == null || Number.isNaN(metrics[metric]));

  for (const metric of unmeasured) {
    blockers.push(
      `${metric} was not measured — an unmeasured gate is not a passing one; run the `
      + 'measurement before advancing'
    );
  }
