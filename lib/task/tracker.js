...
function triggerNextPdcaAction(feature, currentPhase, context = {}) {
  ...
  const matchRate = matchRateRules.isMeasured(context.matchRate) ? context.matchRate : null;
  const threshold = getConfig('pdca.matchRateThreshold', 90);

  let nextPhase;
  if (currentPhase === 'check') {
    /*
     * v2.1.34 — three outcomes, not two.
     *
     * `context.matchRate || 0` turned "no measurement" into 0, which then took
     * the `< threshold` branch and advanced the cycle to `act`. So a cycle that
     * had never run gap analysis would start auto-fixing against nothing,
     * burning iterations to close a gap no one had measured. Staying in `check`
     * is the honest move: the missing step is the measurement itself.
     */
    if (!matchRateRules.isMeasured(matchRate)) {
      nextPhase = 'check';
    } else {
      nextPhase = matchRateRules.allowsAdvance(matchRate, threshold) ? 'report' : 'act';
    }
