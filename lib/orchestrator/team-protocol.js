 ...
 *
 * Required for sub-agent Task tool spawn. bkit dispatches one level by
 * convention; that used to also be a Claude Code constraint, but v2.1.219
 * re-enabled nested spawning to depth 3 by default, so the single level is now
 * bkit's own discipline rather than something the platform enforces (ENH-372).
 *
 * @returns {boolean}
 */
function canSpawn() {
...
function registerSpawn(req) {
  ...
  try {
    ...
    if (typeof sw.addTeammate === 'function') {
      // ENH-379: addTeammate takes a single descriptor object, not positional
      // (name, status, meta) arguments. The old call passed `req.sub` — a
      // string — as `teammateInfo`, so `teammateInfo.name` was undefined and the
      // roster would have gained a nameless row that completeSpawn() could never
      // find again. Latent rather than live: registerSpawn currently has no
      // runtime caller (only lib/orchestrator/index.js re-exports it and
      // test/contract/orchestrator.test.js asserts its return shape), and the
      // existing tests only checked `typeof addTeammate === 'function'`, so
      // neither surfaced the mismatch.
      sw.addTeammate({
        name: req.sub,
        role: req.sub,
        currentTask: req.phase || null,
      });
    }
