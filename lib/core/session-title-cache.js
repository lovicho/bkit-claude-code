...
function isSameAsCached(store, { sessionId, feature, phase, action }) {
  ...
  if (!rec) return false;
  /*
   * ENH-419 (v2.1.33), Issue #77 — stop the republish ping-pong.
   *
   * `action` used to take part in this comparison, and that alone made the
   * title rewrite itself on nearly every turn:
   *
   *   a skill/agent Stop hook publishes with action 'PLAN' | 'ACT' | …
   *   the next user prompt calls generateSessionTitle({ sessionId }) with no
   *     action at all, so action is null
   *   null !== 'PLAN' -> cache miss -> republish
   *   the next skill stop -> cache miss again -> republish
   *
   * So merely alternating between working and typing rewrote the title, which
   * is what users reported as "bkit keeps renaming my session". The feature and
   * phase are what the title is actually about; a title should be reissued when
   * the work moves on, not when the caller happens to know which action it was.
   *
   * `action` is still stored for diagnostics — it just no longer votes.
   */
