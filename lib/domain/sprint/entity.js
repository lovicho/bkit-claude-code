...
function createSprint(input) {
  ...
  return {
    ...
    autoPause: {
      /*
       * ENH-454 (v2.1.36): the caller may supply the armed set, so a configured
       * `sprint.autoPause.armedTriggers` can finally reach a sprint. The read
       * itself stays outside this module — the domain layer has no requires and
       * keeps none.
       *
       * A non-empty array of strings is the only accepted override. An empty
       * array falls back to the full default set rather than disarming every
       * safety trigger, which is the failure mode this release exists to remove.
       */
      armed: (Array.isArray(input.autoPauseArmed)
        && input.autoPauseArmed.length > 0
        && input.autoPauseArmed.every((t) => typeof t === 'string' && t.length > 0))
        ? [...input.autoPauseArmed]
