...
const EXPECTED_COUNTS = Object.freeze({
  ...
  // v2.1.34: these are now counts of hook events/blocks that a host-integration
  // test proves Claude Code actually dispatches — not counts of what is merely
  // registered. FileChanged left the file because it could never fire (see
  // test/contract/deprecation-registry.json → hookEvents.FileChanged), taking
  // the event count 22 → 21 and the block count 25 → 24. Two PostToolUse and
  // one PreToolUse handler were added within existing blocks (one `if` rule per
  // tool, since `if` accepts no `|` alternation), which changes handler count
  // but not block count.
  hookEvents: 21,
  hookBlocks: 24, // SessionStart 1 + PreToolUse 2 + PostToolUse 3 + 18 single-block events
  mcpServers: 2,
  ...
  // v2.1.33 (D1): audit-logger ACTION_TYPES joins the counts SoT. It was the one
  // count that escaped this module, so its drift went unnoticed until three
  // sources disagreed — docs said 19, test/unit/audit-logger.test.js AL-007
  // asserted 29, lib/audit/audit-logger.js shipped 40. Maintainer ruling: the
  // implementation is the source of truth; test and docs follow.
  // v2.1.37 (ENH-432): 41 → 40. `post_tool_block_recorded` was removed — nothing
  // ever wrote it, and it existed for a block decision bkit cannot make (see
  // lib/audit/audit-logger.js at the removal site).
  actionTypes: 40,
});
...
const EXPECTED_HOOK_EVENT_NAMES = Object.freeze([
  'ConfigChange', 'CwdChanged', 'InstructionsLoaded', 'Notification',
