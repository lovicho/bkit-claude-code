 ...
 * Plan SC: G-W2 (all 6 Port↔Adapter mappings complete)
 *
...
function detectCCVersion() {
  ...
  // best-effort: ask the CLI. ENH-427/428 (v2.1.35): shared, shell-free
  // implementation — the `2>/dev/null` this used to shell out for is what
  // `stdio: ['ignore','pipe','ignore']` already does.
  try {
    const { detectViaSubprocess } = require('./cc-version-checker');
    const detected = detectViaSubprocess(2000);
